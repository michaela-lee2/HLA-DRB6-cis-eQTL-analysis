...
| 3    | `03_identify_lead_snp.py`     | Identify the SNP with the minimum P-value from the results (lead SNP: rs9270694)       |
| 4    | `04_ld_fine_mapping.sh`       | Calculate LD (R²) within ±500 kb of the lead SNP and extract candidates with R²≥0.8    |
| 5    | `05_locuszoom_plot.R`         | Generate a LocusZoom plot                                                              |
| 6    | `06_functional_annotation.py` | Functional annotation using the RegulomeDB / HaploReg APIs (motif changes, RFX5, etc.) |
| 7    | `07_specificity_boxplots.R`   | Boxplots of DRB6 vs DRB1 expression by genotype (0/1/2)                                |
| 8    | `08_colocalization.R`         | Colocalization (coloc) analysis of DRB6-DRB1 eQTL signals                              |
| 9    | `09_trans_eqtl_analysis.sh`   | Genome-wide trans-eQTL search (P<1e-12)                                                |
| 10   | `10_pearson_correlation.R`    | Pearson correlation analysis of DRB6-DRB1 expression (r=0.821)                         |
| 11   | `11_ppi_network_string.py`    | Construct an DRB1-centered PPI network using the STRING API                            |
| 12   | `12_enrichment_enrichr.py`    | KEGG2021/Reactome 2024 enrichment analysis using the Enrichr API (FDR<0.05)            |
| 13   | `13_summary_plots.R`          | Summary plots, including Manhattan plots and expression histograms                     |
